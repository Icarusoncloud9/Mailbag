...
export class Worker {
    ...
    public async updateContact(inID: string, inName: string, inEmail: string): Promise<string> {
        const response: AxiosResponse = await axios.put(`${config.serverAddress}/contacts/${inID}/${inName}/${inEmail}`)
        return response.data;
    }
